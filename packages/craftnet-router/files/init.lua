-- craftnet-router: the Customer Router role.
--
...
function package.withPackages(packages)
  assert(type(packages) == "table", "craftnet-router needs its dependencies")
  ...
  assert(type(protocol) == "table" and type(protocol.validate) == "table",
    "craftnet-router requires the craftnet-protocol package")
  assert(type(core) == "table" and type(core.newEngine) == "function",
    "craftnet-router requires a craftnet-core bound to that protocol")
  assert(type(runtime) == "table" and type(runtime.new) == "function",
    "craftnet-router requires a craftnet-runtime bound to that core")
  assert(protocol.wireVersion == package.wireVersion,
    "craftnet-router speaks wire version " .. package.wireVersion
  ...
  local function internal(name)
    ...
    assert(chunk, "craftnet-router cannot load module '" .. name .. "': " .. tostring(loadError))
