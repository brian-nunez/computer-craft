...
function package.withPackages(packages)
  assert(type(packages) == "table", "craftnet-isp needs its dependencies")
  ...
  assert(type(protocol) == "table" and type(protocol.validate) == "table",
    "craftnet-isp requires the craftnet-protocol package")
  assert(type(core) == "table" and type(core.newEngine) == "function",
    "craftnet-isp requires a craftnet-core bound to that protocol")
  assert(type(runtime) == "table" and type(runtime.new) == "function",
    "craftnet-isp requires a craftnet-runtime bound to that core")
  assert(protocol.wireVersion == package.wireVersion,
    "craftnet-isp speaks wire version " .. package.wireVersion
  ...
  local function internal(name)
    ...
    assert(chunk, "craftnet-isp cannot load module '" .. name .. "': " .. tostring(loadError))
