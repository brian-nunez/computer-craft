-- craftnet-computer: the Computer role.
--
...
local package = {
  name = "craftnet-computer",
  version = "0.1.0",
  wireVersion = 1,
}

-- withPackages binds this role to its dependencies. Nothing here locates a
-- package on disk; a startup program or a wizard does that once and hands the
-- results in.
function package.withPackages(packages)
  assert(type(packages) == "table", "craftnet-computer needs its dependencies")
  ...
  assert(type(protocol) == "table" and type(protocol.validate) == "table",
    "craftnet-computer requires the craftnet-protocol package")
  assert(type(core) == "table" and type(core.newEngine) == "function",
    "craftnet-computer requires a craftnet-core bound to that protocol")
  assert(type(runtime) == "table" and type(runtime.new) == "function",
    "craftnet-computer requires a craftnet-runtime bound to that core")
  assert(protocol.wireVersion == package.wireVersion,
    "craftnet-computer speaks wire version " .. package.wireVersion
  ...
  local function internal(name)
    ...
    assert(chunk, "craftnet-computer cannot load module '" .. name .. "': " .. tostring(loadError))
  ...
  local computer = internal("computer")
  local api = {
    name = package.name,
  ...
  for key, value in pairs(computer) do
